...
MODULE epsmethanol_module
    IMPLICIT NONE
    CONTAINS

    FUNCTION calculate_epsmethanol(wp, i) RESULT(epsmethanol)
        IMPLICIT NONE
        DOUBLE PRECISION, INTENT(IN) :: wp
        INTEGER, INTENT(IN) :: i
        DOUBLE PRECISION :: epsmethanol
        DOUBLE PRECISION, DIMENSION(7) :: metc, metw

        metc = (/ 4.38E-1,3.24E-1,1.38E-1,1.20E-1,4.45E-1,2.52E-1,6.49E-2 /)
        metw = (/6.10E-3,1.93E-2,1.02E-1,4.35E-1,1.08E1,2.01E1,3.83E1 /)*(1.602177E-19/1.0545E-34)

        IF (i > 0) THEN
           epsmethanol = 1.D0 + SUM(metc / (1.D0 + (wp /metw)**2.D0))
        ELSE
            epsmethanol = 32.9D0
        END IF
    END FUNCTION calculate_epsmethanol

    END MODULE epsmethanol_module
...
END MODULE epspolystyrene_module
  ...
  USE epsmethanol_module
  IMPLICIT NONE
  DOUBLE PRECISION :: wp, epstoluene, epsmagnetite, epsmethanol,epsteflon, epspolystyrene, eps_mix, G3  !< dilectric functions 
  DOUBLE PRECISION :: dist   !< distance between the particles
  REAL(KIND=16) :: bsum   !< energy sum
  ...
  DOUBLE PRECISION :: diam !< diameter of the nano-particle in nm
  DOUBLE PRECISION :: RadiusPS  !radius polystyrene sphere in m
...
h=1.054571800D-34  !=\hbar

pi = 3.141592653589793D0 !< pi
c=2.99792458D8     !<velocity of light 

RadiusPS=3.5D-6  !radius of polystyrene sphere attached to AFM tip. (Interacting across magnetic fluid with PTFE plane

!vary phi and diam
phi = 0.015D0   !volume fraction of magnetite from 0 to 0.015
diam=16D0  !in nm. vary magnetite diameter between 2 and 23nm

mu2 = 1.0D0
mu4 = 1.0D0
kmax=8000

OPEN(UNIT=10, FILE='results.csv', STATUS='REPLACE', ACTION='WRITE')   !< open file for writing
...
DO dloop=1,200
  ...
  DO i = 0, 2000
    ...
    epsmethanol=calculate_epsmethanol(wp,i)
...
! WRITE(10, '(I5,1X,ES15.8E3,1X,ES15.8E3,1X,ES15.8E3,1X,ES15.8E3,1X,ES15.8E3)') i, wp, epstoluene, epsmagnetite, 
!epsteflon,& epspolystyrene
...
G3=(phi*(epsmagnetite-1.D0)/(epsmagnetite+2))+(1-phi)*(epsmethanol-1.D0)/(epsmethanol+2)
  ...
  END DO
!WRITE(*, '(F10.2, A, ES25.18E3)') dist * 1D9, ' nm, bsum =', bsum
WRITE(*,*) dist * 1D9, ' nm', 2.D0*pi*bsum*1D6, ' \muN/m'
WRITE(20,*) dist * 1D9, ' nm', 2.D0*pi*RadiusPS*bsum*1D6, ' \muN'
WRITE(30,*) dist * 1D9, ' nm', 2.D0*pi*bsum*1D6, ' \muN/m'  !this ought to be best to show in Figures
